...
def export_trades_python(trades, filename=None):
    """Export trades as Python list for use in analyzer"""
    filename = filename or str(LOGS_DIR / 'trades_parsed.py')
...
def export_trades_csv(trades, filename=None):
    """Export trades as CSV"""
    if not trades:
        return

    filename = filename or str(LOGS_DIR / 'trades_parsed.csv')
    keys = trades[0].keys()
