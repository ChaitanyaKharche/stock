...
def skip(label: str, why: str) -> None:
    """A check this host cannot run is SKIP, never FAIL.

    Check 4 asks the OS for a real wake lock, which only Windows can grant. Scored as a
    failure it made this whole file exit 1 on every non-Windows host and in CI -- the
    same defect `preflight_gate_test` had, and the same one PROJECT_REPORT.md names: a
    gate above the attainable range is BROKEN, not strict. A suite that is always red
    stops being read.
    """
...
def main() -> int:
    ...
    if sys.platform != "win32":
        # The lock is a Win32 SetThreadExecutionState call; nothing else can grant it.
        # The guard's OWN non-Windows return value is the thing being reported here, so
        # asserting it says "HELD" off Windows asks the code to lie.
        skip("guard held", f"Windows-only ({sys.platform}); "
                           f"run on the lab machine to check this")
    else:
        state = A.hold_system_awake()
        A.release_system_awake()
        check("guard held", state.startswith("HELD"), state)

    tail = f"  ({len(SKIPPED)} skipped: not this platform)" if SKIPPED else ""
    print("\n" + ("ALL CHECKS PASSED" + tail if not FAILURES
                  else f"{len(FAILURES)} FAILED: {', '.join(FAILURES)}{tail}"))
