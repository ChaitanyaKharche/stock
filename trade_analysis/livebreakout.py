...
class LiveBreakoutTrader:
    ...
    multi-day support/resistance confluence, and a gap/volatility regime filter.

    KEY DIFFERENCE: Don't enter on first breakout signal.
    Instead: wait for 9:30-9:45 confirmation that buyers are serious,
    score that confirmation against multi-day S/R confluence, and size
    risk off current volatility (ATR) instead of fixed percentages.
    """
    ...
    def place_entry_order(self):
        ...
        try:
            ...
            print(f"  Shares: {num_shares} (size x{self.size_multiplier:.2f}, regime: {self.gap_regime})")
    ...
    def run_market_day(self):
        ...
        print(f"Confluence levels tracked: {len(self.confluence_levels)}")
...
if __name__ == "__main__":
    ...
    Multi-Day S/R Confluence + Gap/Volatility Regime Filter
    ...
    - Breakouts that align with a 3-day support/resistance zone get
      sized up (confluence); breakouts during an elevated gap regime
      (headline/news-driven) get sized down instead of held to a static level
