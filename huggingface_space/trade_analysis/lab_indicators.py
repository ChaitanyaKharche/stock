"""VERBATIM COPY of trade_analysis/live_lab/indicators.py -- do not edit here.

The live lab and this Space are separate deployments with separate dependency sets, so
this is a COPY rather than an import: the Space must stay installable from its own
requirements.txt without pulling in the lab, and nothing done here can reach back and
perturb a frozen forward test. If the lab's version changes, re-copy it wholesale rather
than patching this file, and re-run indicators_test.py.

It is dependency-free by design (no pandas, no `ta`), which is exactly why it travels
well. Used here as the reference implementation for Wilder ADX/ATR/RSI -- the Space's
own indicators.py falls back to constants (ADX_9 = 25.0) whenever pandas_ta fails to
import, and `adx > 25` can never be true when adx is exactly 25.0.
